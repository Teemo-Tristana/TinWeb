...
+ 组成:分为8个模块
    + Main模块 : 服务器主函数
        + 作用 : 负责把各个模块组合协调在一起工作, 调用I/O复用(**这里用的是epoll**)监听各个事件,并对各事件调用相应的处理模块
        + 事件 :
            - 事件一 : 新连接请求, 判断条件 sockfd == listenfd
                - 处理: 接受请求,保存用户数据(socketaddr地址, connfd等), 然后将connfd注册到epoll的内核表中,监听**读事件**
            
            - 事件二 : 对端关闭, 判断条件 EPOLLRDHUP | EPOLLHUP | EPOLLERR
                - 处理 : 关闭该连接,并释放该连接的资源
            
            - 事件三 : 读事件且是信号事件, 判读条件 (sockfd == pipefd[0]) && (events[i].events & EPOLLIN))[作用见日志模块]
                - 处理 : 对信号进行处理
            
            - 事件四 : 读事件且是客户端发送请求报文, 判断条件 EPOLLIN
                - 处理 : 将数据读入读缓冲区,然后该任务插入线程池的任务队列中,再由任务队列进行处理

            - 事件五 : 写事件, 判断条件 EPOLLOUT ,我们的web服务器只有一种写事件,就是工作线程处理完请求报文且封装成响应报文放入写数据缓冲区
                - 处理 : 调用函数将响应报文发送给客户端( 浏览器)
        
    + epoll模块 : epoll相关函数

    + http模块  : **最关键模块**,提供http请求及处理返回http响应的相关函数

    + lock模块  : 提同步工具(互斥锁/条件变量/信号量)
        + 作用 : 封装了线程同步工具 信号量类, 互斥量类, 条件变量类

    + threadpoll模块 : 线程池提供工作线程处理http请求
        + 作用 : 提供线程,从工作队列中取任务并执行
        + 关键成员:
            - pthread_t m_threads 线程数组
                - 本线程数组比较简单,线程数组固定,没有对线程动态增删
                - 构建线程池时,创建指定个数线程,存储在线程数组中
                - 初始化创建线程时,把每个线程detach,是线程的属性为unjoinable状态,这样线程结束时就直接释放资源,
                -  核心函数run(): 不断循环直到条件变了唤醒,然后从工作队列头部取出任务并执行(在process()中).

            - list<T*> m_workqueue 请求队列: 存储任务的队列
                - 主循环(main)函数中把任务放入队列尾
                - 工作线程取任务时需要竞争(这里使用的是条件变量)
                - 竞争成功的线程(进程池中的线程)从队列首取取任务并执行


    + log模块  : 记录日志
    + timer模块: 负责定时清理长时间无反应的连接
    + Mysql模块 : 提供数据库服务
