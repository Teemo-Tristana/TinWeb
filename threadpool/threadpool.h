...
template <typename T> //将任务添加带工作队列, T是任务类
class threadpool
{
private:
    int m_thread_number; //线程池中线程的数量
    int m_max_requests;  //请求队列最大允许的请求数量
    bool m_stop;         //是否结束线程

    pthread_t *m_threads;       //进程池数组起始地址,数组大小为 m_thread_number
    std::list<T *> m_workqueue; //请求队列.我们这里用list来存储队列

    locker m_queuelocker;        //保护请求队列的互斥锁(申请队列时竞争的资源)
    sem m_queuestat;             //信号量 通知是否有任务需要处理
...
private:
    ...
    static void *worker(void *arg); //工作线程运行函数,不断从工作队列中取任务并执行
    void run();
};
...
threadpool<T>::threadpool(connection_pool *connPool, int thread_number, int max_requests) : m_thread_number(thread_number), m_max_requests(max_requests), m_threads(nullptr), m_stop(false), m_connPool(connPool)
{
    ...
    for (int i = 0; i < thread_number; ++i)
    {
        ...
        /*1.worker是类的静态成员函数,不能直接是一个类的成员,因而将this指针作为参数
        传递给它,这样就可以使用了类的成员
        */
        ...
        /*设置为脱离线程后,在线程结束时便可以自动释放所有资源,不用在单独回收*/
...
/**
 *  通过list容器创建请求队列,先申请互斥锁(保证线程安全),再向队列添加,
 *  最后通过信号量通知有任务待处理并释放锁
 */
template <typename T>
bool threadpool<T>::append(T *request)
{
    m_queuelocker.lock(); //队列被所有线程共享,属于临界资源,操作时必须加锁
    ...
    m_queuelocker.unlock();
    m_queuestat.post(); //利用信号量通知有新任务待处理
    return true;
}
...
/**
 * 运行函数,不断探寻工作队列是否有待执行任务,若有,取下来执行,否则等待
 * 执行任务: 从工作队列中取出任务并进行处理
 * 注意线程同步
*/
template <typename T>
